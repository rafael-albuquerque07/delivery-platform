...
| Build local falha em `build-logic` — `compilePluginsBlocks`, `compileKotlin`, `jar`, ponto diferente a cada tentativa | **Diagnóstico aberto, não receita — atualizado em 30/08 depois de nove workflows provarem que o `build-logic` em si está bom.** Nesse dia os nove pipelines de serviço compilaram limpos num runner Linux (2m14s a 2m51s), então a causa é desta máquina, não do módulo. A mitigação que esta linha recomendava antes — repositório e `GRADLE_USER_HOME` fora do perfil do usuário — já estava aplicada quando o sintoma apareceu de novo; não é prevenção suficiente sozinha. `gradlew --stop` não limpa a casa: derruba o daemon do Gradle e deixa vivo o do compilador Kotlin — chegaram a sobreviver três JVMs do JDK 21 por horas. Matá-las à força deixa registro de sessão órfão, e a execução seguinte falha com `Unknown or invalid session`; limpar `%LOCALAPPDATA%\kotlin\daemon` resolve esse resíduo específico. `--no-daemon` piora localmente — multiplica daemon de uso único sem derrubar o do Kotlin; no CI é o comportamento certo, aqui não. O que fez o build passar uma vez: matar todas as JVMs, apagar `backend\.gradle` (cache de configuração) e rodar **com** daemon. **Isso não está provado como cura** — com `--rerun-tasks` a falha voltou, com acessores não resolvidos e `Unable to parse script-resolver-environment`. A extensão Java do VS Code mantém cerca de seis JVMs vivas escrevendo em `build/` e é suspeita não testada |
