...
O modelo de valores da **ADR-009** (`subtotalDosItens`, `gorjeta`, `desconto`,
sem `deliveryFee`) e `estoqueControladoSnapshot` no item do pedido **já
renomeados** — em 26/08/2026 e 30/08/2026, incluindo as citações em outras
ADRs. Saem da tabela porque não são mais exceção pendente: são o nome vigente.
